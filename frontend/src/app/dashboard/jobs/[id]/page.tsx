...
import { useJob, useShortlistJob, useGenerateTailored, useDeepScore, useTailorBullets } from "@/hooks/use-api";
import type { TailoredBullet } from "@/hooks/use-api";
...
/* ── Bullet tailoring ────────────────────────────────────────────────────
 * "Take a job description, re-rank + re-word my resume bullets for it."
 * One-click panel that runs Claude (Opus) over the user's bullet bank
 * for THIS job. Shows side-by-side original vs tailored, ⭐ relevance,
 * one-line "why it matches", and a per-row Copy button.
 *
 * Reuses everything we already ship:
 *   - bullets extracted by the resume parser (CandidateBullet rows)
 *   - the user's writing samples for voice (<style_examples> blocks)
 *   - JD entities the scorer already pulled (skills, requirements, keywords)
 */
function RelevanceStars({ value }: { value: number }) {
  if (value <= 0) {
    return <span className="text-[10px] uppercase tracking-wider text-muted-foreground/60">unranked</span>;
  }
  return (
...
function BulletTailoringCard({ jobId }: { jobId: string }) {
  ...
  return (
    <Card>
        ...
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div>
            <CardTitle className="text-base flex items-center gap-2">
              ...
              Tailor my bullets for this role
            </CardTitle>
            <CardDescription className="mt-1">
              Re-rank and rewrite your resume bullets to emphasize what THIS job cares about.
              Voice-matched to your writing samples; never invents facts.
            </CardDescription>
          </div>
          <Button
            size="sm"
      ...
      <CardContent>
        {tailor.isError && (
          <p className="text-sm text-destructive">{tailor.error?.message || "Tailoring failed"}</p>
        )}
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          {/* Bullet tailoring — runs Claude (Opus) over the user's bullet
              bank to rank + rewrite each one for THIS job's requirements.
              Reuses writing samples for voice and the existing bullets
              extracted by the resume parser. */}
          <BulletTailoringCard jobId={id} />
        </div>
