...
async def admin_stale_jobs_verify(
    ...
    """Probe each unapplied job's apply_url and mark only the
    confirmed-dead ones as expired. Far more accurate than the
    time-based cleanup (which catches 30-day-old jobs even if they're
    still open) and complementary — the user reported 8-day-old
    postings already going dead, so a fixed cutoff alone misleads.

    Verification rules (conservative — never false-positive a live job):
      - HTTP 404 or 410       → mark expired
      - HTTP 200/30x          → alive, leave alone
      - HTTP 403/503/timeout  → ambiguous (anti-bot, rate-limit, server
                                hiccup), leave alone
      - Network errors        → ambiguous, leave alone

    Vercel-friendly batching: capped at `limit` jobs per call (default
    100). Probes run concurrently with a semaphore so we don't hammer
    any one host. Returns `has_more=True` when there are still
    unverified rows so the UI can chain calls.
    """
    import asyncio
    import httpx
    from sqlalchemy import select
    ...
    # Pull oldest first — they're the most likely to be dead. Skip jobs
    # that don't have a URL to probe at all.
    q = (
        select(Job)
        .where(
            Job.status.in_(pre_applied),
    ...
    async def probe(client: httpx.AsyncClient, url: str) -> str:
        """Returns 'dead' | 'alive' | 'ambiguous'."""
        try:
            async with sem:
                # Try HEAD first — cheap. Some servers don't allow HEAD,
                # so fall back to GET on 405 / 501.
                ...
                if 200 <= r.status_code < 400:
                    return "alive"
                # 403 / 5xx / 429 — we don't know, don't penalize the row.
                return "ambiguous"
    ...
    async with httpx.AsyncClient(
        ...
        async def check_one(job: Job) -> tuple[Job, str]:
            url = job.apply_url or job.job_url or ""
            verdict = await probe(client, url) if url else "ambiguous"
            return job, verdict
    ...
    # Estimate has_more: another batch of unprobed rows still exists in
    # the bucket. Conservative — if the caller hit the limit, assume yes.
    has_more = len(rows) >= min(max(int(limit), 1), 250)
