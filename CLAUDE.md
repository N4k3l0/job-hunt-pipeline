...
- Jobs a user has worked on (tailored, tracked, Apply for me, saved) never expire on their own, so once the app last saw one listed more than 21 days ago the API adds `closed_note` ("This job may have closed...", `services/job_freshness.py`) and the card shows it. A job never listed again counts from when it was found
